...
router.post("/forgetPassword", async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      const url = `https://node-day-5-fe.netlify.app/reset-password/${randomString}`;
...
router.post("/resetPasswordCheck", async (req, res) => {
  try { 
    console.log("before");
    if (req.body.token) { 
    console.log("aft");
  ...
  try {
    if (req.headers.authorization) {
      ...
      if (string) {
        ...
        let strng = await randomStringModel.findOneAndDelete({
